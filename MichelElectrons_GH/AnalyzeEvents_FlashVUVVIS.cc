...
private:
  ...
  //Numero de fotones detectados (directos y reflejados)
  std:: vector <double> phot_detected;
  std:: vector <double> phot_detectedref;

  //Flashes: numero de fotones reconstruidos y su canal optico
  ...
  std::vector<double> _flash_pe_vuvvis;
...
{ //Esto es una comprobacion y sale cada vez que cambiamos de evento (cuando haya más de un evento)
 ...
 std::cout<<"--------------------------------------------------------------------"<<std::endl;
 //Sacamos los fotones generados, su x, y, z en cada step y el  step
 ...
 for (auto const& phots : *simphotons){
   ...
   // if((6<=opt&&opt<=17)||(60<=opt&&opt<=71)||(84<=opt&&opt<=95)||(138<=opt&&opt<=149)||(170<=opt&&opt<=181)||(224<=opt&&opt<=235)||(248<=opt&&opt<=259)||(302<=opt&&opt<=313)){
     int  dp=0;
 ...
 //Fotones reflejados  detectados: SimPhotonsLite
 //Bucle sobre los 320 canales
 for (auto const& photsref : *simphotonsref) {
   ...
   Total_photdetectedref +=dp;
    
   std::cout<<"chanopt_ref= "<<opt<<std::endl;  
    std::cout<<"phot_detected_ref VIS = "<<dp<<std::endl;
   
   // }//if
 }

 std::cout<<"----------------------------------------------------------"<<std::endl;
 std::cout<<"Fotones reflejados(VIS) detectados totales= "<<Total_photdetectedref<<std::endl;

 std::cout<<"----------------------------------------------------------"<<std::endl;
 std::cout<<"phdetectadosVIS/MeV= "<<Total_photdetected/Total_Energydeposit<<std::endl;


 _flash_pe_vuvvis.clear();
 ...
 // for (size_t l = 0;l < _flash_label_v.size(); l++) {
   ...
   if(flashsize>1){flashsize=1;}
    for (int i = 0; i < flashsize; i++) {
     ...
     const Int_t n=_flash_pe_vuvvis.size();
     ...
     for(size_t j=0; j < _flash_pe_vuvvis.size(); j++){
       hitsperchan.push_back(vhitsperchan[j]);
       std::cout<<"Canal: "<<j<<" Numero de fotones dig flash= "<<_flash_pe_vuvvis[j]<<std::endl;
...
void test::AnalyzeEvents::beginJob()
{
  ...
  fTree->Branch("flash_pe_vuvvis", &_flash_pe_vuvvis);
