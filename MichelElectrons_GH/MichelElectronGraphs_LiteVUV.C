...
for(int j=0; j<9; j++) {
 ...
 //G4 10000 Michels con distribucion de energía gaussiana centrada en 30MeV
 //distribucion de posciones gaussiana en x y z en el centro del detector
...
for(int i=0;i<entriesMichels;i++) //numero de particulas que lanzas son las entries (10000)
{
  treeMichels->GetEntry(i); //primer electron de Michel


  cout<<"------Electron numero-----"<<i<<endl;
  //Inicializamos el numero de fotones generados y la energia para cada electron a 0
  double phot_gen=0;
  double Energy=0;
...
//de las deposiciones de energia y calculamos la media
   //Para que las posiciones estén pesadas con la energía
...
//Calculamos la media de las posiciones
  ...
  //Calculamos la distancia de la posicion del Michel a cada PMT y el numero de fotones detectados en cada PMT para ese punto de centelleo
...
////////////
   ...
   for(int k=0;k<chanopt->size();k++){
     double chan=chanopt->at(k);
     if(chan==chanpmt)
     {

       double hits_simulation= phot_detected->at(k);
...
//El 0 es como la direccion del vector
 gr[i] = new TGraphErrors(vx.size(), &vx[0], &vy[0], 0, &vey[0]);
}


//Hacemos un graph grande sobre el que pintamos los demas xx tamaño del eje x e yy es el tamaño del eje y
double xx[2]={0,600};
double yy[2]={0,1.5};
...
graph->GetYaxis()->SetTitle("N_{Lite}/N_{#Omega}/cos(#theta)");
//la "a" se pone en este graph para que pinte los axis y la p puntos
graph->Draw("ap");
...
gr[0]->GetYaxis()->SetTitle("N_{Lite}/N_{#Omega}/cos(#theta)");
gr[0]->Draw("p");
